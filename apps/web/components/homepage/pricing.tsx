...
const PricingHeader = ({ title, subtitle }: PricingHeaderProps) => (
  <div className="text-center mb-10">
    ...
    <p className="text-gray-600 dark:text-gray-300 mt-4 max-w-2xl mx-auto">{subtitle}</p>
  </div>
);
...
const PricingCard = ({ user, isYearly, name, prices, description, benefits }: PricingCardProps) => {
  const router = useRouter();
  const getProCheckoutUrl = useAction(api.subscriptions.getProOnboardingCheckoutUrl);

  const currentPrice =
    prices.find((price) => (isYearly ? price.recurringInterval === "year" : price.recurringInterval === "month")) ||
    prices[0];
  let priceAmount = "0.00";
  if (currentPrice.amountType !== "free") {
    priceAmount = currentPrice ? (currentPrice.priceAmount / 100).toFixed(2) : "0";
  }
  ...
  const buttonText = !user ? "Sign in to continue" : !currentPrice ? "No price available" : "Get Started";
...
export default function Pricing({ result }: PricingProps) {
  ...
  const filteredProducts = result.items.filter((item) =>
    item.prices?.map((price) => (isYearly ? price.recurringInterval === "year" : price.recurringInterval === "month")),
  );
  ...
  return (
    <section className="px-4 py-16">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {filteredProducts.toReversed().map((item) => (
            <PricingCard
              key={item.id}
              user={user}
              name={item.name}
